...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,com.aenlly.Reportknife";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("com.aenlly.Reportknife", 1, 9, "公主链接报刀", "1.0.0", 1, "Aenlly", "酷Q样例应用(V9应用机制)", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("com.aenlly.Reportknife", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("com.aenlly.Reportknife", AppData.CQLog);	
